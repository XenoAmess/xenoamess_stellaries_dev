...
| R2 | 第一次接触事件 | 选中科研舰队，以 `effect random_owned_ship = { ship_event = { id = graygoo.400 scopes = { from = orbit } } }` 建立有效 ship/from scope | 450×150 新图正确显示，无拉伸、裁切异常或黑块 |
...
2026-09-13 在 Steam 版 Stellaris `Pegasus v4.4.6 (fdde)`、简体中文、2560×1440、仅启用本 Mod 的隔离 userdir 中完成 R1—R9。实际结果如下：

| 编号 | 结果 | 证据摘要 |
| --- | --- | --- |
| R1 | 通过 | 主菜单与新开局正常加载，Mod 身份与版本正确。 |
| R2 | 通过 | 使用第 5.2 节的有效 ship/from scope 命令触发 `graygoo.400`，第一次接触专用图正确显示。 |
| R3 | 通过 | `graygoo.401`—`406` 全链逐页检查；透明肖像与原版房间合成正常，无黑边、白边或黑块。 |
| R4 | 通过 | 运行 `create_gray_official = yes` 后触发 `graygoo.501`；灰风官员使用新肖像。 |
| R5 | 通过 | `graygoo.511` 专用战败图正确显示。 |
| R6 | 通过 | 经 `graygoo.499` 建立人物后触发 `graygoo.512`；回归图与人物肖像均正确，下缘构图在事件窗口中可接受。 |
| R7 | 通过 | 创建并重载 `xeno_gray_runtime01.01.sav` 后再次触发回归事件，肖像和事件图保持正确。存档为 1,156,063 bytes，SHA-256 `75f19b53588e2e76b8bb1eef7f6210fcfe332c6c9f0c48fac50b2d40629dede1`。 |
| R8 | 通过 | 触发仍使用 `GFX_evt_ship_in_orbit_2` 的原版 `distar.270`，显示仍为原版飞船图。 |
| R9 | 通过 | 第二次干净启动/退出的 `error.log` 无本 Mod 错误；`game.log` 只记录版本、defines 与 interface defines。 |

验收运行 ID 为 `20260913T093054Z`。候选包树 SHA-256 为 `35609a568a8add1f12eb979b5e4a620f7469d06d0dbe5c6b600caf7255172c2d`，游戏可执行文件 SHA-256 为 `bc451c72d9654c8901f1bb0bee1dd78d76f415465c2fbf746e9f98ade333173a`。正式版只把版本从 `1.0.0-rc.1` 改为 `1.0.0`，不改脚本或美术；发布前的正式版包树 SHA-256 为 `5a3b29c31ab84ca7f08e88b962abd5bb0db971f80c982d2321846bee00d02350`。
...
`1.0.0` 把用户原指令中的方括号误当成名称正文，导致游戏与 Steam 显示为 `[XenoAmess的灰风美化]`。`1.0.1` 只把正式名称修正为 `XenoAmess的灰风美化`，并同步 VERSION、descriptor、发布合同、Workshop 标题/说明、Changelog、Change Note 和本机外层描述符。

本次不修改任何事件脚本、资源引用或图片字节。由于 `1.0.0` 已成功上传，不复用旧版本号；同一公开物品 `3800996999` 使用新版本 `1.0.1` 更新。

### 10.2 验收与发布门禁

1. 专项测试必须断言无方括号名称、版本为 `1.0.1`、Workshop ID 仍为 `3800996999`，且全部美术哈希和脚本最小差异测试继续通过。
2. 此变更只涉及元数据，不重新执行 P 语言或实机矩阵；`1.0.0` 的 R1—R9 结果仍作为相同脚本与美术字节的运行时证据。
3. Steam Change Note 固定为：`[v1.0.1] 修正 Mod 展示名称，移除误加入名称正文的方括号；脚本、美术和兼容范围不变。`
4. 更新后匿名回读必须确认公开标题和说明标题均为 `XenoAmess的灰风美化`；SteamCMD 下载包必须只在 `descriptor.mod` 的版本和名称上与 `1.0.0` 不同。
5. 验收完成后提交并推送仓库，创建并推送 `gray-wind-v1.0.1` 标签。

### 10.3 实际状态

- 2026-09-13 17:38:14 UTC，Steamworks 在用户发出暂停后续发布指令之前已经返回更新成功；远端 `3800996999` 因此实际已成为 `1.0.1`，没有再执行任何后续远端修改。
- 匿名详情回读标题为 `XenoAmess的灰风美化`、`visibility=0`、`banned=0`，内容大小 2,277,238 bytes，manifest 为 `7572862430904898836`；`[v1.0.1]` Change Note 可见。
- 匿名 SteamCMD 下载得到 13 个文件，与 staging 逐文件一致；相对 `1.0.0` 只有 `descriptor.mod` 改变，其版本为 `1.0.1`、名称无方括号，全部脚本与美术字节不变。
- 用户随后明确要求停止为此事项继续发布；本轮在记录已发生状态并推送仓库后终止，不再操作远端。
