...
- 当前公开正式版本：`1.0.1`；Workshop 物品为 `3797257579`。
- 当前待发布正式版本：`1.1.0`，用于交付 Stellaris 官方 10 种语言全量本地化及迭代 1 的全部回归收口结果；仅简体中文进行运行时验收，其他语言只做静态校验。
...
- `1.1.0` 发布前必须保持计划 00—13 的玩法脚本不变，完成 10 种语言静态合同、S03/S04 计划 10 重复叠加闭环、I1-R01 蜂巢岗位闭环，并把完整说明与 Change Note 更新到同一版本。
