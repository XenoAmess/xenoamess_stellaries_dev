...
- I1-003 简体中文正反向实机门禁已经完成；`1.0.1` 在静态门禁、仓库测试、隐藏物品内容复核和文案逐字校验通过后公开。
...
5. 从干净 Git 提交构造上传目录；首次发布必须创建新 Workshop 物品，绝不更新只读上游 `3710613857`。
...
本项目取得新 Workshop ID 后，后续版本只更新该新物品。上游 `3710613857` 永久保持只读；首次创建新物品前必须从待上传描述符移除其 `remote_file_id`，新 ID 只能在 Steam 成功创建后写回。
