...
如果需求边界定义为“只替换直接呈现小灰本人形象的素材”，则游戏内必交素材只有 **1 张人物肖像**：`800 × 350`、透明背景、未压缩 32-bit A8R8G8B8、straight alpha、无 mip。现有 `gray.dds` 的同一张图会被以下位置复用：

- `graygoo.401`—`406` 初次通讯直接指定自定义 portrait；
- `graygoo.499` 创建小灰国家首领后固定其 portrait；
- `create_gray_official` 每次重建行政官形态后再次固定 portrait；
- `graygoo.500`、`502`、`503`、`512` 从小灰国家 ruler 读取同一 portrait；
- `graygoo.501` 从重建的 `gray_official` 读取同一 portrait；
- 领袖列表、领袖详情、内阁和通知继续裁切复用该 portrait。

在这个严格范围内，`areta_005_room.dds` 是环境背景，不是人物素材；`ship_in_orbit_2.dds` 的首次发现画面没有出现小灰；击溃/归来事件横幅、旗帜、特质图标、陆军图标、战舰和运输舰也都排除。若还要翻新 Workshop 发布页，则在游戏内 1 张肖像之外另做 `thumbnail.png` 和展示图，但它们是发布素材，不是游戏内人物素材。

如果“人物直接相关”允许包含小灰专属身份 UI，而不要求画面必须出现人物，则可把 `leader_trait_governor_gray` 的 `29 × 29` 特质图标作为第 2 张；它是唯一合理的边界扩展。旗帜和陆军图标仍分别属于国家与单位形态，不纳入人物立绘包。
