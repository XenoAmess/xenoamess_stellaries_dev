...
3. `I1-R02` 第三方 UI 冲突回归尚未形成独立收口记录。仅启用本 Mod 的长菜单、展开和收起基线已经通过；仍需把该证据映射到 R02，并在需要时选取一个已知 UI Mod 记录“兼容或明确冲突”，第三方 Mod 自身修复不在范围内。
4. 初始综合夹具中的 `S01`、`S03`、`S04`、`S05`、`S06` 仍有历史 RED、follow-up 或未执行状态：需用当前树刷新精确单 Mod 加载；补计划10岗位上限断言；执行计划10重复；完成蜂巢/机械/机仆/游牧岗位矩阵映射；在 I1-004 后完成英文静态矩阵。已通过的 I1-001—003 专项证据可以复用，但必须明确回链，不能静默把旧状态改绿。
5. Steam 新物品 `3797257579` 已创建但仍为隐藏草稿。v1.0.1 的新标题、canonical `workshop/description.bbcode`、changelog 和上传 staging 已准备且通过静态门禁；按用户 2026-09-07 指示暂不继续发布。后续需经用户审阅后上传 v1.0.1、更新标题/完整说明/Change Note，从空缓存核验文件树，再改为公开并完成 API 核验、仓库 ID 回写和 `v1.0.1` Git 标签。
