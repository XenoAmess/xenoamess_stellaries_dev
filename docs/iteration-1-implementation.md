...
1. `I1-004` 全量英文本地化尚未实现。当前只有 I1-003 新增功能依赖的六个英文键通过静态校验；计划 00—12 的完整决议、地块、说明和效果文本仍需翻译并做静态验收，按项目规则不启动英文实机。
2. `I1-R01` 蜂巢岗位可就业回归尚未执行。需要用简体中文蜂巢帝国验证科研与行政岗位能被人口填充、名称/类别/产出正确，并检查新日志。
3. 初始综合夹具中的 `S01`、`S03`、`S04`、`S05`、`S06` 仍有历史 RED、follow-up 或未执行状态：需用当前树刷新精确单 Mod 加载；补计划10岗位上限断言；执行计划10重复；完成蜂巢/机械/机仆/游牧岗位矩阵映射；在 I1-004 后完成英文静态矩阵。已通过的 I1-001—003 专项证据可以复用，但必须明确回链，不能静默把旧状态改绿。
