...
1. `I1-R01` 蜂巢岗位可就业回归尚未执行。需要用简体中文蜂巢帝国验证科研与行政岗位能被人口填充、名称/类别/产出正确，并检查新日志。
2. 初始综合夹具中的 `S01`、`S03`、`S04`、`S05` 仍有历史 RED、follow-up 或未执行状态：需用当前树刷新精确单 Mod 加载；补计划10岗位上限断言；执行计划10重复；完成蜂巢/机械/机仆/游牧岗位矩阵映射。`S06` 已以“简体中文运行时通过、9 种非中文翻译静态通过”收口。已通过的 I1-001—004 专项证据可以复用，但必须明确回链，不能静默把旧状态改绿。
