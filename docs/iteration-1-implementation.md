...
- 当前阶段：实现及静态验收已完成，正在执行 Stellaris 4.4.6 简体中文实机回归；首轮执行、岗位容量、重复执行、存档与重载断言已通过，剩余普通/非机仆帝国的反向可见性。英文及其他语言按项目范围只做翻译和静态校验。
...
6. 原实现阶段曾把描述符版本推进至 `4.4.6-i1.3`；版本规范建立后改用独立 SemVer，当前发布候选为 `1.0.0-rc.1`，Stellaris 兼容范围仍由 `supported_version="4.4.*"` 单独表达。同步更新 Mod 树哈希、机器可读合同和场景夹具。
...
- 当前仍不能把 I1-003 标为完成：尚需在简体中文普通/非机仆帝国中验证入口不可见。本功能六个英文必需键只进入静态本地化合同，不再启动英文环境。
