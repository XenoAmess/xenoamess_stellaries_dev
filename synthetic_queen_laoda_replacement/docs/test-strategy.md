...
| S5 | 包隔离与版本 | 候选版为 `0.1.0-rc.1`；首发 `0.1.1`。当前缩略图更新版 `VERSION`/描述符/changelog 均为 `0.1.2`。正式 `mod/` 只有四个白名单文件；肖像定义与 DDS 字节不变；不含其他 Mod 文件或旧 Workshop ID。缩略图与用户 ZIP 内 PNG 字节一致。 |
