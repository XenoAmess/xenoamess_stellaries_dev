"""Publish the Laoda Synthetic Queen item with the installed Steamworks DLL.
...
APP_ID = 281990
VERSION = "0.1.1"
TITLE = "XenoAmess的合成女王替换·牢大"
UPSTREAM_ID = 3710613857
OTHER_SYNTHETIC_QUEEN_ID = 3807768508
ROOT = Path(__file__).resolve().parents[1]
REPO = ROOT.parent
MOD = ROOT / "mod"
WORKSHOP = ROOT / "workshop"
STATE = ROOT / "evidence/v0.1.1/publish-state.json"
DLL = Path(r"C:\Program Files (x86)\Steam\steamapps\common\Stellaris\steam_api64.dll")
MEDIA = (
    WORKSHOP / "media/01-children-returned.jpg",
    WORKSHOP / "media/02-first-dialogue.jpg",
)
...
def preflight() -> dict:
    ...
    note = (WORKSHOP / "change-note-v0.1.1.txt").read_text(encoding="utf-8").strip()
    ...
    if len(re.findall(rb"\[img\].*?\[/img\]", description)) != len(MEDIA):
        raise RuntimeError("BBCode must contain both in-game screenshot links")
    ...
    if sha256(MOD / "gfx/models/portraits/xenoamess_cetana_laoda_portrait.dds") != "a35c0431498c6b8511bed0604517ba629695edad86e0556c277019d0bbbbf071":
    ...
    if any(sha256(image) != sha256(source) for image, source in zip(MEDIA, accepted)):
        raise RuntimeError("Workshop screenshots differ from Steam F12 evidence")
...
def publish(inputs: dict) -> dict:
    ...
    steam = Steam()
    state = {"schema": "laoda-synthetic-queen-workshop-v1", "created_at_utc": datetime.now(timezone.utc).isoformat(), "inputs": inputs}
    try:
        ...
        if created.result != 1 or not created.item_id or created.item_id in (UPSTREAM_ID, OTHER_SYNTHETIC_QUEEN_ID):
        ...
        checked("SetItemPreview", os.fsencode(str(MEDIA[0].resolve())))
        ...
        state["staged_preview_files"] = {"primary": MEDIA[0].name, "additional": [p.name for p in MEDIA[1:]]}
        save_state(state)
        note = (WORKSHOP / "change-note-v0.1.1.txt").read_text(encoding="utf-8").strip()
...
def main() -> int:
    ...
    parser.add_argument("--connection-check", action="store_true", help="check the logged-in Steam session without creating an item")
